...
export default function TeacherProfile() {
  ...
  return (
    <div dir={isRTL ? 'rtl' : 'ltr'} className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-blue-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 py-8 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Tabs */}
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md mb-6 p-2 flex gap-2 border border-gray-100 dark:border-gray-700">
          <TabButton
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2">
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-6 border border-gray-100 dark:border-gray-700 mb-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">{t('basicInfo')}</h2>
            {!editMode ? (
              <button
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-6 border border-gray-100 dark:border-gray-700">
          <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4">{t('subjects')}</h3>
          <div className="space-y-2">
            {teacher?.subjects?.map((subject: Subject) => (
              <div key={subject.id} className="flex items-center gap-3 p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
        ...
        <div className="bg-gradient-to-br from-purple-600 to-blue-600 rounded-2xl shadow-lg p-6 text-white">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="dark:text-white">{t('status')}</span>
              <span className="bg-white dark:bg-slate-800 bg-opacity-20 px-3 py-1 rounded-full text-sm font-semibold">
                {teacher?.status === 'active' ? t('active') : t('inactive')}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="dark:text-white">{t('gender')}</span>
              <span className="dark:text-white">{teacher?.gender === 'male' ? t('male') : t('female')}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="dark:text-white">{t('totalMinutes')}</span>
              <span className="font-bold dark:text-white">{teacher?.total_minutes}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="grid md:grid-cols-2 gap-6">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-6 border border-gray-100 dark:border-gray-700">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h4 className="text-sm text-gray-600 dark:text-gray-400">{t('totalEarnings')}</h4>
            <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{wallet?.totalEarnings} {currency}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-6 border border-gray-100 dark:border-gray-700">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h4 className="text-sm text-gray-600 dark:text-gray-400">{t('paidAmount')}</h4>
            <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{wallet?.paidAmount} {currency}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-6 border border-gray-100 dark:border-gray-700">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h4 className="text-sm text-gray-600 dark:text-gray-400">{t('pendingAmount')}</h4>
            <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">{wallet?.pendingAmount} {currency}</p>
          </div>
        </div>
      </div>
    </div>
...
function TransactionsTab({ transactions, currency, t }: { transactions: Transaction[]; currency: string; t: any }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700">
      <div className="p-6 border-b border-gray-100 dark:border-gray-700">
        <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">{t('transactionsHistory')}</h2>
        <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">{t('transactionsDesc')}</p>
      </div>
...
function TransactionItem({ transaction, currency, t }: { transaction: Transaction; currency: string; t: any }) {
  ...
  return (
    <div className="p-6 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-900 transition">
        ...
        <div className="flex items-start gap-4 flex-1">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className={`text-xs px-2 py-1 rounded-full font-semibold ${statusColors[transaction.status]} dark:bg-opacity-30`}>
                {statusLabels[transaction.status]}
              </span>
            </div>
            
            {transaction.reason && (
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">{transaction.reason}</p>
            )}
            
            {transaction.reject_reason && (
              <div className="flex items-start gap-2 p-3 bg-red-50 dark:bg-red-900/20 rounded-lg mb-2">
            ...
            {transaction.approve_reason && (
              <div className="flex items-start gap-2 p-3 bg-green-50 dark:bg-green-900/20 rounded-lg mb-2">
            ...
            <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
              <span className="dark:text-white">{t('balanceBefore')} {transaction.balance_before} {currency}</span>
              <span className="mx-2">←</span>
              <span className="dark:text-white">{t('balanceAfter')} {transaction.balance_after} {currency}</span>
            </div>
          </div>
        </div>
...
function StatCard({ icon: Icon, label, value, color }: any) {
  return (
    <div className={`${color} dark:bg-opacity-20 rounded-xl shadow-md p-5 border border-gray-100 dark:border-gray-700`}>
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm font-semibold dark:text-gray-100">{label}</span>
...
function TabButton({ active, onClick, icon: Icon, label }: any) {
  return (
    ...
    >
      <Icon className="w-5 h-5" />
      <span className="hidden md:inline dark:text-white">{label}</span>
    </button>
  );
}
    ...
    <div>
      ...
      <div className="relative">
        ...
        <input
          ...
          onChange={onChange}
          disabled={disabled}
          className={`w-full ${isRTL ? 'pl-10 pr-4' : 'pr-10 pl-4'} py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-purple-500 focus:border-transparent disabled:bg-gray-100 dark:disabled:bg-slate-800 disabled:text-gray-600 dark:disabled:text-gray-400`}
        />
      </div>
    </div>
  );
...
function ActionCard({ icon: Icon, title, description, onClick, color, isRTL }: any) {
  return (
    <button
      onClick={onClick}
      className={`w-full bg-white dark:bg-slate-800 rounded-xl shadow-md hover:shadow-lg transition-all p-6 text-start border border-gray-100 dark:border-gray-700 group`}
    >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-purple-500 focus:border-transparent"
              required
            />
          </div>
